...
`vercel.json` は現在 `/works/:id` を `/index.html` に rewrite します。新しいクライアントルートを増やす場合は、直接アクセス時にも SPA が返るよう rewrite を更新します。
...
1. API 入力値の検証と問い合わせのレート制限・bot 対策を追加する。
2. API と画面のテストを追加し、CI で lint/build/test を実行する。
3. 一覧 API の並び順、ページネーション、ローディング・エラー・空状態を明示する。
4. 作品 API の本番・開発実装で共通の変換関数を利用し、差分を防ぐ。
5. 全 SPA ルートの直接アクセスと 404 方針を整理する。
6. 固定メールアドレスを環境変数化し、環境ごとの誤送信を防ぐ。
